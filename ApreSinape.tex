...
\item
  ...
  \prod_{k=0}^{K-1} \pi(\mu_k)\pi(\sigma^2_k)
  \end{aligned}
  \]
\item
...
\tightlist
\item
  \textbf{Initialization}: Noise parameters \((\mu_k, \sigma^2_k)\) are
...
\item
  ...
  a near-infrared image was taken from FONTE.
\item
  \textbf{Analysis:} Log-PL, random walks, distributions and visual
  comparisons.
\item
  \textbf{Prioris:}
...
\includegraphics[width=\linewidth,height=0.35\textheight,keepaspectratio]{Apreimages/plot_Application_LogPL.png}

}

\subcaption{\label{fig-logpl}Log pseudolikelihood}
...
\includegraphics[width=\linewidth,height=0.35\textheight,keepaspectratio]{Apreimages/plot_Application_Convergence.png}

}

\subcaption{\label{fig-convergence}MCMC traces}
...
\includegraphics[width=\linewidth,height=0.35\textheight,keepaspectratio]{Apreimages/plot_Application_Distributions.png}

}

\subcaption{\label{fig-distributions}Posterior distributions}
