...
\hypersetup{
  pdftitle={Markov Random Fields with Spacial Seeds},
  pdfauthor={Wesley Cabral Alves; Victor Freguglia Souza},
  hidelinks,
  pdfcreator={LaTeX via pandoc}}


\title{Markov Random Fields with Spacial Seeds}
...
\begin{frame}{The Spacial Limitation}
\phantomsection\label{the-spacial-limitation}
\begin{itemize}
\tightlist
\item
  Many real images and textures possess spacial patterns we call
  \textbf{``seeds''} that may attract or repel colors.
\item
  \textbf{Our Proposal:} Incorporate spacial seeds that exert radial
...
\item
  \textbf{Spacial Transition:} That ``Null Boundary'' radius gives to
...
\item
  ...
  \item
    \textbf{Spacial:} Seed position \(s_j \sim \text{Unif}(L)\)
...
\item
  ...
  are used among the spacial parameters prioris to generate the initial
  \(Z^{(0)}\) hidden field.
\item
...
\item
  ...
  \(Z\).
\item
  \textbf{Fast Convergence:} After step 30001, parameters did not change
...
\item
  ...
  spacially complex HMRFs viable and robust.
\item
