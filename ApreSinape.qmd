...
0. **Initialization**: Noise parameters $(\mu_k, \sigma^2_k)$ are calculated via K-means over the observed intensity $Y_i$ and they are used among the spacial parameters prioris to generate the initial $Z^{(0)}$ hidden field.
...
4. **Step D (Gibbs - Noise):** Sample new $(\mu_k, \sigma^2_k)$ directly from closed-form analytical distributions.
...
* **The test:** In order to evaluate our model in real world data, a near-infrared image was taken from FONTE.
* **Analysis:** Log-PL, random walks, distributions and visual comparisons.
* **Prioris:**
...
::: {#fig-parameters-panel layout="[[1], [1, 1]]"}
![Log pseudolikelihood](Apreimages/plot_Application_LogPL.png){#fig-logpl height="35%"}

![MCMC traces](Apreimages/plot_Application_Convergence.png){#fig-convergence height="35%"}

![Posterior distributions](Apreimages/plot_Application_Distributions.png){#fig-distributions height="35%"}
:::
